...
function removeFirstOccurrences(str, value) {
  ...
  if (idx !== -1) {
    return str.slice(0, idx) + str.slice(value.length + idx);
  }
  return str;
}
...
function sumOfCodes(str) {
  ...
  return charCode.reduce((acc, char) => acc + char, 0);
}
...
function reverseString(str) {
  ...
  return String(reversedArr.join(''));
}
...
function findLongestWord(sentence) {
  ...
  separatedSentence.forEach((word) => {
    const clearedWord = word.replace(/[.,/#!$%^&*;:{}=\-_`~()?]/g, '');
    if (clearedWord.length > maxLength) {
      maxLength = clearedWord.length;
      longestWord = clearedWord;
    }
  });
  return longestWord;
}
...
function reverseWords(str) {
  ...
  const result = separatedStr.map((word) => word.split('').reverse().join(''));
  return result.join(' ');
}
...
function invertCase(str) {
  const separatedStr = str.split('');
  const result = separatedStr.map((letter) => {
...
function getCardId(value) {
  const suits = '♣️♦️♥️♠️';
  const suitIndex = suits.indexOf(value.substring(value.length - 1));
  const faceValue = value.substring(0, value.length - 1);
  let cardRank = 0;
  if (value.length === 4) {
    return suitIndex * 13 + parseInt(faceValue, 10);
  }
  switch (faceValue) {
    case 'A':
      cardRank = 0;
    ...
    case 'K':
      ...
      break;
    default:
      cardRank = parseInt(faceValue, 10) - 1;
      break;
  }
  return (suitIndex / 2) * 13 + cardRank;
}
